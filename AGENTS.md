...
`src/styles/tokens.css` defines all color / spacing / radius / typography custom properties (`--bg`, `--surface`, `--accent` warm copper, `--space-1..8` on a 4px scale, `--font-sans` Inter Variable, `--font-mono` JetBrains Mono Variable, with `prefers-color-scheme: light` overrides). It is imported from `main.jsx` before `index.css`. New CSS must use these tokens — no hex literals or hand-rolled gradients. The fretboard/diagram SVGs also draw from theme-aware tokens (`--fret-board-bg`, `--fret-string`, `--fret-string-active`, `--fret-marker`, `--diagram-line`, `--diagram-string`, `--diagram-fret-label`): dark defaults are white-translucent (built for the dark board) and would vanish on the light board, so light overrides swap them to dark warm-toned ink. Reference these via inline `style={{ fill/stroke: 'var(--token)' }}` in the SVG JSX — never as raw `fill=`/`stroke=` presentation attributes (see the Lucide/SVG note below: `var()` does not resolve there). Any new hardcoded color in `Fretboard.jsx` / `GuitarString.jsx` / `ChordDiagram.jsx` must be tokenized the same way or it will break one theme.
